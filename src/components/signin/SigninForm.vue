<template>
  ...
  >
    ...
    <el-form ref="form" :model="form" :rules="rules" :label-position="labelPosition" label-width="105px" @submit.prevent.native="checkEnter">
      ...
      <el-form-item>
        <el-button size="small" type="primary" @click="checkForm('form')" :loading="sending">報到</el-button>
...
<script>
import { updSubSignApi, chkMemberApi } from '../../api'
...
export default {
  data () {
    return {
      ...
      rules: {
        gcroup: [{ required: true, message: '請選擇組別', trigger: 'change' }],
  ...
  methods: {
    ...
    async checkMember () {
      try {
        ...
        const { IsChgShow, ApiTitle, DataChangeMsg, MID } = response.data
    ...
    async submitForm () {
      try {
        const response = await this.$http.post(updSubSignApi, {
        ...
        if (IsApiError) {
          ...
          if (this.form.cacheInfo) {
            ...
            this.$cookie.set('clc-sign-gcroup', this.form.gcroup, {
              expires: date
            })
            this.$cookie.set('clc-sign-group', this.form.group, {
              expires: date
            })
            this.$cookie.set('clc-sign-name', this.form.name, {
              expires: date
            })
            this.$cookie.set('clc-sign-phone', this.form.phone, {
              expires: date
            })
            this.$cookie.set('clc-sign-email', this.form.email, {
              expires: date
            })
          } else {
